    ...
    class Package
      class SmartOS < Chef::Provider::Package
        ...
        def candidate_version
          ...
          IO.popen(" pkgin search #{@new_resource.package_name}") do |ver|
            vers = []
        ...
        def install_package(name, version)
          ...
          shell_out!( "pkgin -y install #{package.split(' ').first}", :env => nil)
        end
        ...
        def remove_package(name, version)
          ...
          package = "#{name}-#{version}"
          shell_out!("pkgin -y remove #{package}", :env => nil)
        end
      end
    end
  end
      ...
      class Smartos < Chef::Provider::User::Useradd
        ...
        def check_lock
          ...
          unless status.exitstatus == 0
            ...
            if status.exitstatus == 1 && platform_family?('rehl')
